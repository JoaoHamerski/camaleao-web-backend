...
<div class="row mt-2">
    ...
    <div class="col-md-9">
        <x-card header-color="primary" icon="fas fa-edit">
            <x-slot name="header">Alterar dados do pedido</x-slot>
            <x-slot name="body">
                <order-form :is-edit="true" order-id="{{ $order->id }}"
                    client-id="{{ $order->client ? $order->client->id : null }}"
                    :has-client="{{ !!$client ? 'true' : 'false' }}" />
            </x-slot>
        </x-card>
    </div>
</div>
