<x-card header-color="success" icon="fas fa-user">
    ...
    <x-slot name="body">
        ...
        <div class="@if ($client->getTotalOwing() > 0) text-danger @else text-success @endif font-weight-bold">Total
            devendo: </div>

        <div class="@if($client->getTotalOwing() > 0) text-danger @else text-success @endif">{!!
            Mask::money($client->getTotalOwing(), 2) !!}</div>
    </x-slot>
    ...
    <x-slot name="footer">
        <div class="d-flex flex-column">
            ...
            <a class="text-danger" href="#clientDeleteModal" data-toggle="modal">
    ...
    @else
    <x-slot name="body">
        <div class="text-secondary font-weight-bold text-center">
            PEDIDO SEM CLIENTE
        </div>
    </x-slot>
    @endif
...
@if (Request::routeIs('clients.show'))
<client-modal :is-edit="true" :id="{{ $client->id}}" ref="clientModal"></client-modal>
